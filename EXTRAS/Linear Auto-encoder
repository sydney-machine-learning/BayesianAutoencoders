...
#hyper parameters
batch_size = 512 #no of training examples in one forward/backward pass
epochs = 20 #no of passes through complete training set
learning_rate = 1e-3
encoding_dim = 0
use_dataset = int(input("Enter dataset to use: 1. MNIST 2. STL-10 3. CIFAR10     ")) #1. MNIST 2.STL-10 3.Flickr8k
shape = 0
...
if(use_dataset==1):
    ...
    train_dataset = torchvision.datasets.STL10(root="~/torch_datasets", split='train', transform=transform, download=True)
    test_dataset = torchvision.datasets.STL10(root="~/torch_datasets", split='test', transform=transform, download=True)
    encoding_dim=2048
    shape=96
    img_shape=96*96
elif(use_dataset==3):
    train_dataset = torchvision.datasets.CIFAR10(root="~/torch_datasets", train=True, transform=transform, target_transform=None, download=True)
    test_dataset = torchvision.datasets.CIFAR10(root="~/torch_datasets", train=False, transform=transform, target_transform=None, download=True)
    encoding_dim=256
    shape=32
    img_shape=32*32
